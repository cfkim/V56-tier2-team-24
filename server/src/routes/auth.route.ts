    ...
    try {
        ...
        if (!emailSent) {
            ...
            if (process.env.NODE_ENV === "development") {
                return res.json({
                    message:
                        "Password reset link generated successfully (development mode)",
                    email: email,
            ...
            res.status(200).json({
                message: `password link successfully sent to ${email}`,
            });
        }
    } catch (error) {
...
authRoutes.post("/password/reset", async (req, res) => {
    try {
        ...
        const valid = await ResetPasswordModel.findOne({
            _id: code,
